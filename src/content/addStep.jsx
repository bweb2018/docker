  ...
  renderBaseStepList(filteredSteps) {
    ...
    return (
        <ul className={'list pl4'}>
          ...
          <i 
            className={C({'dn': stepToEdit && stepToEdit.type === item.type},
            "fr pointer mr4 f7 ms-Icon ms-Icon--ChevronDownMed")}
            aria-hidden="true" 
            onClick={(e)=>{
              e.stopPropagation();
              this._addSteps(item)
           ...
           />
           <i 
            className={C({'dn': !stepToEdit || stepToEdit && stepToEdit.type !== item.type},
            "fr pointer mr4 f7 ms-Icon ms-Icon--ChevronUpMed")}
            aria-hidden="true" 
            onClick={(e)=>{
              e.stopPropagation();
