...
class EditStep extends React.Component {
  ...
  isDelStep = (steps)=> {
    const {
      type, onSwitchContent, addSteps, content, runSteps, enSteps, stepToEdit
    } = this.props;
  ...
  render() {
    ...
    const { onSddSteps, content, config } = this.props
    const baseStep = editingStep ? getStep(editingStep.type) : '';
    return (
        baseStep ? <div className={C({'ml4':content.type === 'edit_run_step' || content.type === 'edit_entrypoint_step'},"c-st mv2 pt1")}>
