...
import {validateConfig} from "../config-utils";
    ...
    _clicked () {
      ...
      const { items, props:{addSteps, addIcon, baseDockers} } = th
      if (text === 'Apply') {
        th.setState({
          isShow: false,
          isAdd: true,
      ...
      return (
        <ul className='list pl4 pr1'>
          {dockers.map((item, index) => <li className={'cb pv3 bt b--near-white '} key={index}><span className={C({'blue-custom':docker.name === item.name},'pointer h3 mv7')}  
            ...
            <i className={C({'dn':docker.name !== item.name},'v-mid ml3 ms-Icon ms-Icon--StatusCircleOuter blue-custom')} />
            <i className={C({'dn':docker.name !== item.name},'v-top nl2_9 ms-Icon ms-Icon--StatusCircleCheckmark white')} />
            <i className= {C({"dn":index === id,},"pointer mr4 fr f7 ms-Icon ms-Icon--ChevronDownMed")}
             onClick= {() => { this.items= item, this.onPresetChosen(item,index)}}
            />
            <i className={C({'dn':index !== id,},"pointer mr4 fr f7 ms-Icon ms-Icon--ChevronUpMed")}
          ...
          <li className={'cb pv3 bt-m bt bb b--near-white'}><span  className={C({'blue-custom':docker==='Custom'},'h3 mv7 pointer')}
            ...
            <i className={C({'dn':docker !== 'Custom'},'v-mid ml3 ms-Icon ms-Icon--StatusCircleOuter blue-custom')} />
            <i className={C({'dn':docker !== 'Custom'},'v-top nl2_9 ms-Icon ms-Icon--StatusCircleCheckmark white')} />
            <i className= {C({"dn":id === 4,},"pointer mr4 fr f7 ms-Icon ms-Icon--ChevronDownMed")}
             onClick= {() => { this.items= 'Custom', this.onPresetChosen('Custom',4)}}
            />
            <i className={C({'dn':id !== 4,},"pointer mr4 fr f7 ms-Icon ms-Icon--ChevronUpMed")}
            onClick= {()=> this.setState({id:10})}
            />
            { id === 4 ? this.a : <div></div> }
          </li>
